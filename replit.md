...
- **Real-time polling**: All cards refresh every 5 seconds
- **Dark/Light mode**: Toggle in header
- **Logout**: With confirmation dialog

## API Proxy
All frontend requests go to `/api/*` which the Express server proxies to the external cabin API. This avoids CORS issues and keeps the external URL server-side.
